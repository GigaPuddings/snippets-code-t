...
fn migrate_to_markdown_with_db_path(
    ...
    let conn = match db_path_override {
        ...
        None => {
            ...
            DbConnectionManager::get().map_err(|e| format!("数据库连接失败: {}", e))?
        }
    };
        ...
        .query_map([], |row: &Row| {
            Ok(Fragment {
                ...
                updated_at: row
                    ...
                    .unwrap_or_else(|| row.get::<_, String>(4).unwrap()),
...
fn export_fragment(
    ...
    let folder_path = if let Some(cat_id) = fragment.category_id {
        ...
        let category = categories.iter().find(|c| c.id == cat_id).ok_or_else(|| {
    ...
    .to_string();
    let created_str =
        chrono::DateTime::from_timestamp_millis(parse_datetime_to_timestamp(&fragment.created_at))
            .map(|dt| dt.to_rfc3339())
            .unwrap_or_else(|| chrono::Utc::now().to_rfc3339());
    let modified_str =
        chrono::DateTime::from_timestamp_millis(parse_datetime_to_timestamp(&fragment.updated_at))
            .map(|dt| dt.to_rfc3339())
            .unwrap_or_else(|| chrono::Utc::now().to_rfc3339());
    let fm = FrontMatter {
