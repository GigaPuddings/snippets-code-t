...
impl FileSystemManager {
    ...
    pub fn create_markdown_file(
        ...
        fs::write(&file_path, full_content)
            .map_err(|e| map_io_error(&e, "写入文件", &file_path))?;
    ...
    pub fn read_markdown_file_content(&self, file_path: &Path) -> Result<String, String> {
        // 解析为完整路径
    ...
    // * `Err(String)` - 读取失败的错误信息
    pub fn read_markdown_file(&self, file_path: &Path) -> Result<(FrontMatter, String), String> {
        // 解析为完整路径
        let full_path = if file_path.is_absolute() {
            file_path.to_path_buf()
        } else {
        ...
        match (content, metadata) {
            ...
            (None, Some(meta)) => {
                ...
                let full_content = if existing_body.trim().is_empty() {
                    format_frontmatter_block(meta)?.trim_end().to_string()
                } else {
                    format!(
                        "{}\n\n{}",
                        format_frontmatter_block(meta)?.trim_end(),
                        existing_body
                    )
                };
                fs::write(&full_path, full_content)
    ...
    pub fn move_markdown_file(
        ...
        if old_full_path == new_full_path {
            let relative_path = new_full_path
        ...
        // 移动文件
        fs::rename(&old_full_path, &new_full_path).map_err(|e| format!("移动文件失败: {}", e))?;
    ...
    pub fn list_categories(&self) -> Result<Vec<String>, String> {
        let entries =
            fs::read_dir(&self.workspace_root).map_err(|e| format!("读取工作区目录失败: {}", e))?;
    ...
    pub fn list_markdown_files(&self, category: Option<&str>) -> Result<Vec<PathBuf>, String> {
        ...
        match category {
            Some(cat) => {
                ...
                let entries =
                    fs::read_dir(&folder_path).map_err(|e| format!("读取目录失败: {}", e))?;
