...
impl CacheManager {
    ...
    fn from_config_dir(config_dir: PathBuf, silent_missing: bool) -> Result<Self, String> {
        ...
        let cache = match cache_result {
            ...
            Err(e) => {
                warn!(
                    "⚠️ [CacheManager] 读取 cache.json 失败: {}，使用默认配置",
                    e
                );
                CacheConfig::default()
            }
    ...
    pub fn reload_from_disk(&mut self) -> Result<(), String> {
        match read_cache(&self.config_dir) {
            ...
            Err(e) => Err(format!("重新加载 cache.json 失败: {}", e)),
        }
    }
            ...
            if !self.cache.files.contains_key(file_path) {
                ...
                if let Ok(file_meta) = std::fs::metadata(&full_path) {
                    if let Ok(modified) = file_meta.modified() {
                        let file_modified_ts =
                            chrono::DateTime::<chrono::Utc>::from(modified).timestamp_millis();
                        if let Some(metadata) = self.cache.files.get_mut(file_path) {
                            // 仅当文件真实修改时间与缓存不同时才更新
                            if metadata.modified != file_modified_ts {
                                metadata.modified = file_modified_ts;
                                info!(
                                    "🔄 [CacheManager] 文件实际修改时间变化，更新: {}",
                                    file_path
                                );
                            }
                        }
    ...
    pub fn get_or_create_category_id(&mut self, category_name: &str) -> i64 {
        ...
        self.cache
            .categories
            .insert(category_name.to_string(), metadata);
        info!(
            "✨ 创建新分类: {} (ID: {}, is_system: {})",
            category_name, new_id, is_system
        );
    ...
    pub fn ensure_default_categories(&mut self) {
        ...
        let needs_fix_assets = if let Some(metadata) = self.cache.categories.get("assets") {
            ...
            if metadata.id != -1 || !metadata.is_system {
                warn!(
                    "⚠️ 发现\"assets\"的配置不正确，当前 ID: {}, is_system: {}，需要修复",
