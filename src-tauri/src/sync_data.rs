...
mod tests {
    ...
    fn sync_path_policy_rejects_local_and_derived_data() {
        ...
        assert!(!is_allowed_sync_path("node_modules/pkg/README.md", &roots));
    }

    #[test]
    fn attachment_root_never_escapes_workspace() {
        assert_eq!(
            normalized_attachment_root("assets/${noteFileName}/"),
            Some("assets".to_string())
        );
        assert_eq!(
            normalized_attachment_root("../outside/${noteFileName}"),
            None
        );
        assert_eq!(
            normalized_attachment_root(".snippets-code/${noteFileName}"),
            None
        );
        assert_eq!(normalized_attachment_root("C:/Users/demo"), None);
    }

    #[test]
    fn sync_map_only_updates_changed_fields() {
        let initial =
            BTreeMap::from([("general.language".to_string(), serde_json::json!("zh-CN"))]);
        let first = update_sync_map(SyncMapFile::default(), &initial, "device-a");
        let original_clock = first.values["general.language"].clock.clone();
        let unchanged = update_sync_map(first, &initial, "device-b");
        assert_eq!(unchanged.values["general.language"].clock, original_clock);
    }
    ...
    #[test]
    fn staging_only_includes_portable_workspace_data() {
        let temp = TempDir::new().unwrap();
        git(&temp, &["init"]);
        ...
        .unwrap();
        fs::write(
            temp.path().join(".snippets-code").join("app.json"),
            r#"{"workspaceRoot":"C:\\private"}"#,
        )
        .unwrap();
        fs::write(temp.path().join("snippets.db"), b"device database").unwrap();
    ...
    #[test]
    fn pre_staged_local_data_is_removed_from_git_index_but_kept_on_disk() {
        let temp = TempDir::new().unwrap();
        git(&temp, &["init"]);
        git(&temp, &["config", "user.name", "Sync Test"]);
        git(&temp, &["config", "user.email", "sync@example.com"]);
        ...
        .unwrap();
        git(&temp, &["add", "-f", ".snippets-code/app.json"]);
        git(&temp, &["commit", "-m", "legacy local data"]);

        let staged = stage_allowed_sync_changes(temp.path()).unwrap();
        assert!(staged.contains(&".snippets-code/app.json".to_string()));
        assert!(temp
            .path()
            .join(".snippets-code")
            .join("app.json")
        ...
        let output = Command::new("git")
            ...
            .output()
            .unwrap();
        assert!(String::from_utf8_lossy(&output.stdout).contains("D\t.snippets-code/app.json"));
    }

    #[test]
    fn git_status_ignores_untracked_device_data() {
        let temp = TempDir::new().unwrap();
        git(&temp, &["init"]);
        fs::create_dir_all(temp.path().join(".snippets-code")).unwrap();
        fs::write(temp.path().join("snippet.md"), "# portable").unwrap();
        fs::write(
            temp.path().join(".snippets-code").join("app.json"),
            r#"{"workspaceRoot":"C:\\private"}"#,
        )
        ...
        assert!(status
            .changed_files
            .iter()
            .any(|path| path.ends_with("snippet.md")));
        assert!(!status
            .changed_files
            .iter()
    ...
    #[test]
    fn git_status_marks_legacy_device_data_for_index_migration() {
        let temp = TempDir::new().unwrap();
        git(&temp, &["init"]);
        git(&temp, &["config", "user.name", "Sync Test"]);
        git(&temp, &["config", "user.email", "sync@example.com"]);
        ...
        assert!(status
            .changed_files
            .contains(&"D .snippets-code/app.json".to_string()));
        assert!(temp
            .path()
            .join(".snippets-code")
            .join("app.json")
    ...
    #[test]
    fn field_level_merge_keeps_independent_device_changes() {
        ...
        let mut base = SyncMapFile::default();
        base.values.insert(
            "appearance.theme".to_string(),
        ...
        base.values.insert(
            "general.language".to_string(),
            field(serde_json::json!("zh-CN"), "2026-01-01T00:00:00Z", "base"),
        );
        ...
        let merged = merge_sync_map_files(Some(&base), &ours, &theirs).unwrap();
        assert_eq!(
            merged.values["appearance.theme"].value,
            serde_json::json!("dark")
        );
        assert_eq!(
