...
- [ ] 继续评估 `TipTapEditor` 的右键菜单和会话滚动恢复是否需要进一步收敛。
...
3. `[~]` 拆 TipTapEditor：`useEditorViewMode`、`useEditorPersistenceBridge`、`useEditorBacklinks`、`useEditorImageUpload`、`useEditorSearch`、`useEditorOutline` 和 `useEditorLinks` 已落地并覆盖测试，下一步继续收敛右键菜单和会话滚动恢复。
