...
- [ ] 继续评估 `TipTapEditor` 的大纲和右键菜单是否需要进一步收敛到已存在 composable。
...
3. `[~]` 拆 TipTapEditor：`useEditorViewMode`、`useEditorPersistenceBridge`、`useEditorBacklinks`、`useEditorImageUpload` 和 `useEditorSearch` 已落地并覆盖测试，下一步继续收敛大纲和右键菜单。
