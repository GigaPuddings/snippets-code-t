...
2. `[x]` 给搜索和 Markdown 转换补测试：搜索排序、搜索语法解析、语义过滤、搜索结果复制动作、Markdown JSON 转换、wikilink、前端插件权限、Git 自动同步生命周期、Git runtime host/事件监听/窗口自动同步、Git runtime controller、Git 冲突解决动作、Git 冲突弹窗状态/确认流程/结果反馈/流程编排、Git 仓库异常弹窗、Rust frontmatter、Rust 插件校验测试已落地。下一步测试重点转向 Git 冲突弹窗交互和插件安装压缩包解析。
