...
1. `[~]` 先收敛架构边界：产品叙事和架构文档已推进，Git 冲突状态工具、runtime state、冲突弹窗状态、确认流程、结果反馈、仓库异常弹窗、冲突流程编排、runtime host 默认装配、runtime portal、runtime controller、自动同步窗口生命周期、runtime 事件监听和冲突解决动作已迁入插件目录；截图/翻译逻辑仍需继续迁出核心页面。
