...
- [ ] 梳理 `src/pages/config/index.vue` 中 Git 同步状态、事件监听、弹窗和副作用入口。
- [ ] 新增 `src/plugins/git-sync/composables/useGitSyncRuntime.ts` 或等价 controller。
...
- [ ] 增加 Rust 侧 `cargo test` 可执行用例，优先覆盖 Markdown frontmatter 和插件权限。
...
1. `[~]` 先收敛架构边界：产品叙事和架构文档已推进，Git/截图/翻译逻辑仍需继续迁出核心页面。
2. `[~]` 给搜索和 Markdown 转换补测试：搜索排序、搜索语法解析、语义过滤、Markdown JSON 转换、wikilink、插件权限测试已落地，Rust 侧 frontmatter/权限测试待补。
