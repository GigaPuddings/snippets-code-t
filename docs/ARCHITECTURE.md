...
数据事实源、持久状态、派生索引、缓存、清理级联和增量检索的详细边界，见 [数据存储、清理与索引管理方案](./DATA_MANAGEMENT.md)；GitHub/Git 跨设备同步白名单、新设备恢复和配置冲突策略，见 [跨设备数据同步方案](./DATA_SYNC.md)。

## 概述

Snippets Code 是一个基于 Tauri 2 + Vue 3 + Rust 的跨平台桌面应用，主线定位为本地优先的前端代码片段与知识资产管理工具。应用采用前后端分离架构，前端使用 Vue 3 生态系统，后端使用 Rust 提供高性能的系统级能力，截图、翻译、Todo、应用启动器、Git 同步、主题等能力通过官方插件增强。

### 核心特性

- **前端片段管理**: 支持语言、框架、类型等语义元数据，面向组件、Hook、样式、配置、修复记录等前端资产管理
- **知识资产编辑**: 基于 TipTap 的 Markdown 编辑器，兼容笔记与代码片段工作流
- **全文与语义搜索**: 支持片段内容、标签、分类，以及 `lang:`、`framework:`、`kind:` 等前端语义检索
- **分类管理**: 灵活的分类和标签系统
- **官方插件增强**: 截图/OCR/贴图、翻译、Todo、应用启动器、Git 同步、主题等能力以插件形式扩展核心工作台
...
│   │   ├── config/               # 配置页面
│   │   └── ...                   # 核心页面与插件挂载入口
│   ├── plugins/                  # 前端插件系统与官方插件运行时
│   │   ├── git-sync/             # Git 同步插件前端集成
│   │   ├── screenshot/           # 截图/OCR 插件前端集成
│   │   ├── translation/          # 翻译插件前端集成
│   │   ├── todo/                 # Todo 插件前端集成
│   │   └── ...                   # 其他官方插件
│   ├── router/                   # 路由配置
│   ├── store/                    # 状态管理
