...
import { I as St, c as T, d as ne, r as ht, k as ct, ao as we, w as Zt, l as Ft, a as K, o as j, e as H, q as A, F as Tt, h as Rt, n as bt, b as ie, y as me, u as ft, B as xt, s as Wt, f as ye, p as pe, a1 as ve, ah as ke, ag as Gt } from "./runtime-entry-DtonpMUQ.js";
import { W as se } from "./window-CRRXS0fr.js";
import { l as P, f as q, a8 as mt, e as Se, k as oe, h as Vt } from "./_plugin-vue_export-helper-CKv_N83a.js";
import { a as zt, c as Dt, P as be } from "./text-aVsB5hRg.js";
import { W as Me } from "./Write-CwGZkilW.js";
import { T as Ce } from "./TextRecognition-NOP_79O4.js";
import { C as Te } from "./Check-DysukQpp.js";
import { C as Re } from "./Close-Dz9sou1L.js";
const ze = St("arrow-left-up", !0, function(o) {
  return T("svg", {
    width: o.size,
    height: o.size,
    viewBox: "0 0 48 48",
...
}), Ae = St("delete-four", !1, function(o) {
  return T("svg", {
    width: o.size,
    height: o.size,
    viewBox: "0 0 48 48",
...
}), Ie = St("rectangle-one", !1, function(o) {
  return T("svg", {
    width: o.size,
    height: o.size,
    viewBox: "0 0 48 48",
...
class Be {
  ...
  drawColorPicker(t, e) {
    ...
    const b = Math.floor($ / 2), W = v + b * _, D = R + b * _;
    ...
    let L = R + x + 25;
    if (i) {
      this.ctx.fillStyle = i.hex, this.ctx.fillRect(w + 15, L - 13, 16, 16), this.ctx.strokeStyle = "#ccc", this.ctx.strokeRect(w + 15, L - 13, 16, 16), this.ctx.fillStyle = "#333";
      const z = s === "hex" ? i.hex : `${i.rgb.r}, ${i.rgb.g}, ${i.rgb.b}`;
      this.ctx.fillText(z, w + 40, L), L += 20;
    }
    this.ctx.fillStyle = "#666", this.ctx.font = "12px Arial", this.ctx.fillText(`X: ${Math.round(n.x)}, Y: ${Math.round(n.y)}`, w + 15, L), L += 20, this.ctx.font = "12px Arial";
    const Y = l ? "已复制!" : "Q:复制 Shift:切换";
    this.ctx.fillStyle = l ? "#00A000" : "#888", this.ctx.fillText(Y, w + 15, L), this.ctx.restore();
  }
}
class Ee {
  // 物理尺寸
...
const vt = class vt extends It {
  ...
  renderRealMosaic(t, e, n, i) {
    ...
    if (v <= 0 || R <= 0) return;
    const $ = Math.floor(p * r - n.x), _ = Math.floor(x * r - n.y), b = Math.ceil(v * r), W = Math.ceil(R * r), D = t.canvas.width, L = t.canvas.height, Y = Math.max(0, $), z = Math.max(0, _), N = Math.min(b, D - Y), E = Math.min(W, L - z);
    if (N <= 0 || E <= 0) return;
    let O;
    try {
    ...
    for (let X = -ot; X < E; X += c)
      for (let C = -V; C < N; C += c) {
        const B = Math.max(0, C), tt = Math.max(0, X), nt = Math.min(N, C + c), S = Math.min(E, X + c);
        if (B >= nt || tt >= S) continue;
        let J = 0, M = 0, dt = 0, at = 0;
        for (let wt = tt; wt < S; wt++)
          for (let st = B; st < nt; st++) {
            const ut = (wt * N + st) * 4;
            J += U[ut], M += U[ut + 1], dt += U[ut + 2], at++;
          }
        if (at > 0) {
          J = Math.round(J / at), M = Math.round(M / at), dt = Math.round(dt / at);
          for (let wt = tt; wt < S; wt++)
            for (let st = B; st < nt; st++) {
              const ut = (wt * N + st) * 4;
              U[ut] = J, U[ut + 1] = M, U[ut + 2] = dt;
            }
        }
      }
...
class Mt {
  ...
  static async loadOCR() {
    ...
    const e = import("./ocr-IQlvyccX.js").then((n) => (this.loadedModules.set(t, n), this.loadingPromises.delete(t), n));
  ...
  static async loadOfflineTranslator() {
    ...
    if (this.loadingPromises.has(t))
      return this.loadingPromises.get(t);
    const e = import("./_plugin-vue_export-helper-CKv_N83a.js").then((n) => n.bz).then((n) => (this.loadedModules.set(t, n), this.loadingPromises.delete(t), n));
    return this.loadingPromises.set(t, e), e;
  }
  /**
   * 预加载模块（在空闲时）
...
const Ye = /[.!?。！？；;：:][)"'”’】）\]]*$/, Jt = /[\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff\uac00-\ud7af]/, Ot = /[A-Za-z0-9]/, Ue = /^\s*(?:[-*+•·]\s+|\d+[.)、）]\s*|[a-zA-Z][.)]\s+)/;
function Xe(o) {
...
function Je(o, t) {
  const e = [...o].sort((r, l) => r.x - l.x), n = xn(e), i = e.reduce((r, l, c) => {
    const u = l.text.trim();
    return c === 0 ? u : en(r, u);
  }, ""), s = Pt(e.map((r) => Lt(r.fontSize, t))), a = Pt(e.map((r) => de(r)));
  return {
    text: i,
    blocks: e,
    bbox: n,
...
function re(o, t) {
  ...
  return i === "-" && Ot.test(a) && Ot.test(s) ? `${e.slice(0, -1)}${n}` : mn(s) || yn(i) ? `${e}${n}` : Jt.test(i) || Jt.test(s) || ee(i) || ee(s) ? `${e}${n}` : Ot.test(i) && Ot.test(s) ? `${e} ${n}` : `${e} ${n}`.replace(/\s+/g, " ");
}
function jt(o) {
...
function wn(o) {
  return Lt(Pt(o.map((t) => de(t))), 16);
}
function de(o) {
  return Lt(o.lineHeight, Lt(o.height, Lt(o.fontSize, 16) * 1.2));
}
function te(o, t, e, n) {
  const i = t + n;
...
class pn {
  ...
  drawFlowedTranslationBlocks(t, e, n, i, s, a, r) {
    const l = [...e].filter((b) => (b.translatedText || b.text).trim()).sort((b, W) => {
      const D = b.bbox.y - W.bbox.y;
      return Math.abs(D) > 4 ? D : b.bbox.x - W.bbox.x;
    });
    if (l.length === 0)
      return;
    const c = Math.max(24, r - s * 2), u = s, g = (b, W, D) => l.map((L) => {
      const Y = (L.translatedText || L.text).trim(), z = Math.max(12, Math.min(L.fontSize || 16, 48)), N = Math.max(8, Math.round(z * b)), E = L.isCodeBlock ? '"Consolas", "Monaco", monospace' : '"Microsoft YaHei", "PingFang SC", "Hiragino Sans GB", sans-serif', O = Math.max(
        s,
        Math.min(L.bbox.x + s, a * 0.38)
      ), U = W ? s : O, V = Math.max(48, a - U - u), ot = L.lineHeight || N * 1.35, X = Math.max(
        N * (D ? 1.04 : 1.14),
      ...
      const Y = L === b.length - 1 ? 0 : D.paragraphGap;
      return W + D.lines.length * D.lineHeight + Y;
    }, 0);
    let p = g(1, !1, !1), x = w(p);
    if (x > c) {
    ...
    for (const b of p) {
      ...
      for (const W of b.lines)
        t.fillText(W, b.x, _), _ += b.lineHeight;
      _ += b.paragraphGap;
    }
  }
  ...
  sampleRegionColor(t, e = 16) {
    ...
    const c = this.backgroundImage.width, u = this.backgroundImage.height, g = this.canvas.width / i, w = this.canvas.height / i, p = c / g, x = u / w, v = t.x * p, R = t.y * x, $ = t.width * p, _ = t.height * x;
    try {
      l.drawImage(
      ...
      return P.error("[截图] 采样区域绘制失败", C), this.canvasPool.release(r), n;
    }
    let b;
    try {
      b = l.getImageData(0, 0, s, a);
    } catch (C) {
      return P.error("[截图] 获取像素数据失败", C), this.canvasPool.release(r), n;
    }
    const W = b.data, D = Math.ceil(Math.sqrt(e)), L = s / (D + 1), Y = a / (D + 1);
    let z = 0, N = 0, E = 0, O = 0;
    for (let C = 1; C <= D; C++)
      for (let B = 1; B <= D && !(O >= e); B++) {
        const tt = Math.floor(B * L), nt = Math.floor(C * Y);
        if (tt >= 0 && tt < s && nt >= 0 && nt < a) {
          const S = (nt * s + tt) * 4;
          S >= 0 && S + 2 < W.length && (z += W[S], N += W[S + 1], E += W[S + 2], O++);
        }
      }
  ...
  isUniformColor(t, e = 100, n = 16) {
    ...
    try {
      ...
      return P.error("[截图] isUniformColor: 采样区域绘制失败", C), this.canvasPool.release(r), !1;
    }
    let b;
    try {
      b = l.getImageData(0, 0, s, a);
    } catch (C) {
      return P.error("[截图] isUniformColor: 获取像素数据失败", C), this.canvasPool.release(r), !1;
    }
    const W = b.data, D = Math.ceil(Math.sqrt(n)), L = s / (D + 1), Y = a / (D + 1), z = [];
    for (let C = 1; C <= D; C++)
      for (let B = 1; B <= D && !(z.length >= n); B++) {
        const tt = Math.floor(B * L), nt = Math.floor(C * Y);
        if (tt >= 0 && tt < s && nt >= 0 && nt < a) {
          const S = (nt * s + tt) * 4;
          S >= 0 && S + 2 < W.length && z.push({
            r: W[S],
            g: W[S + 1],
            b: W[S + 2]
          });
    ...
    if (this.canvasPool.release(r), z.length < 2)
      return !1;
    const N = z.reduce((C, B) => C + B.r, 0) / z.length, E = z.reduce((C, B) => C + B.g, 0) / z.length, O = z.reduce((C, B) => C + B.b, 0) / z.length, U = z.reduce((C, B) => C + Math.pow(B.r - N, 2), 0) / z.length, V = z.reduce((C, B) => C + Math.pow(B.g - E, 2), 0) / z.length, ot = z.reduce((C, B) => C + Math.pow(B.b - O, 2), 0) / z.length;
    return (U + V + ot) / 3 < e;
  }
  /**
   * 根据OCR块高度估算合适的字体大小
      ...
      try {
        if (!this.backgroundImage) {
          ...
          for (; !this.backgroundImage && J < 3e3; )
            await new Promise((M) => setTimeout(M, 100)), J += 100;
          if (!this.backgroundImage)
            throw new Error("背景图像加载超时");
        }
        const c = window.devicePixelRatio || 1, u = Math.max(0, s.padding || 0), g = this.canvas.width / c, w = this.canvas.height / c, p = t - u, x = e - u, v = n + u * 2, R = i + u * 2, $ = Math.max(1, Math.round(v * c)), _ = Math.max(1, Math.round(R * c)), b = Math.max(0, p), W = Math.max(0, x), D = Math.min(g, p + v), L = Math.min(w, x + R), Y = Math.max(0, D - b), z = Math.max(0, L - W), N = Math.round(b * c), E = Math.round(W * c), O = Math.min(
          this.canvas.width - N,
          Math.round(Y * c)
        ), U = Math.min(
          this.canvas.height - E,
          Math.round(z * c)
        ), V = Math.round((b - p) * c), ot = Math.round((W - x) * c), X = document.createElement("canvas");
        X.width = $, X.height = _;
        ...
        C.imageSmoothingEnabled = !1, s.fillColor && (C.fillStyle = s.fillColor, C.fillRect(0, 0, $, _)), O > 0 && U > 0 && C.drawImage(
          this.backgroundImage,
          N,
          E,
          O,
  ...
  async performOcrTranslation() {
    if (!(!this.selectionRect || !this.backgroundImage))
      try {
        ...
        const R = r * g, $ = l * g, _ = performance.now();
        v.drawImage(
          this.backgroundImage,
          R,
          $,
        ...
        const b = performance.now(), { recognizeFromCanvas: W } = await Mt.loadOCR();
        n = Math.round(performance.now() - b);
        const D = performance.now(), L = await W(x);
        ...
        const z = L.blocks, N = performance.now(), E = Xe(z);
...
`), U = {
        ...
        P.info("[OCR] 翻译语言检测完成", B), mt.log("[OCR] translation language detected", B), Dt(C) && (this.translationOverlay.sourceLanguage = C, this.translationOverlay.targetLanguage = this.getAutoTargetLanguage(C));
        const tt = V === "offline" ? Mt.loadOfflineTranslator() : null, nt = (f) => f.includes("插件") && f.includes("未启用") ? f.includes("translation") ? "OCR识别已完成，但翻译插件未启用，请在插件设置中安装并启用 Translation 插件" : f : V === "offline" ? f.includes("未激活") || f.includes("未下载") || f.includes("运行时未安装") ? f : f.includes("超时") ? "模型加载超时，请重试" : f.includes("离线翻译暂仅支持") ? f : "离线翻译失败: " + f.substring(0, 50) : f.includes("timeout") || f.includes("超时") ? "翻译超时，请检查网络连接" : f.includes("network") || f.includes("网络") || f.includes("请求") || f.includes("令牌") ? "翻译失败，请检查网络连接" : f.trim() ? f.slice(0, 80) : "翻译失败，请检查网络连接", S = async () => {
          ...
          if (k) {
            const [, gt, it, G, kt, Bt] = k, $t = zt(it), ue = M(it) || !Dt($t) ? it : await J(it, $t, this.getAutoTargetLanguage($t)), Nt = zt(kt), ge = Dt(Nt) && !ot(kt) ? await J(kt, Nt, this.getAutoTargetLanguage(Nt)) : kt;
        ...
        const Ht = E.map((f) => this.toDisplayParagraph(f, g)), _t = Ht.map((f) => this.toLegacyOverlayBlock(f));
        this.translationOverlay.blocks = _t, this.translationOverlay.paragraphBlocks = Ht, this.translationOverlay.isLoading = !1, this.translationOverlay.errorMessage = X || void 0;
        const yt = {
          cropDurationMs: e,
          ocrModuleLoadDurationMs: n,
          ocrRecognitionDurationMs: i,
...
const vn = { class: "toolbar-panel first-panel" }, kn = { class: "tool-section" }, Sn = ["onClick", "title"], bn = { class: "action-section" }, Mn = ["disabled", "title"], Cn = ["disabled", "title"], Tn = ["title"], Rn = ["title"], zn = ["title"], An = {
  ...
  key: 0,
  class: "style-group"
}, Wn = { class: "line-width-selector" }, Ln = ["onClick", "title"], In = {
  key: 1,
  ...
  setup(o, { expose: t, emit: e }) {
    const n = ht(!1), i = ht(null), { t: s } = Se(), a = o, r = e, l = ct(() => [
      { type: d.Select, icon: Le, title: s("screenshot.select") },
      { type: d.Rectangle, icon: Ie, title: s("screenshot.rectangle") },
      { type: d.Arrow, icon: ze, title: s("screenshot.arrow") },
      { type: d.Pen, icon: Me, title: s("screenshot.pen") },
      { type: d.Mosaic, icon: We, title: s("screenshot.mosaic") },
      { type: d.Text, icon: De, title: s("screenshot.text") },
      { type: d.ColorPicker, icon: qt, title: s("screenshot.colorPicker") },
      { type: d.Ocr, icon: Ce, title: s("screenshot.ocr") },
      { type: d.Translate, icon: we, title: s("screenshot.translate") },
      { type: d.Pin, icon: be, title: s("screenshot.pin") }
    ]), c = ct(() => [
    ...
    ), $ = ct(() => a.currentTool === d.Text), _ = ct(() => a.currentTool === d.Mosaic), b = ct(() => a.currentTool === d.Translate), W = (S) => r("tool-select", S), D = (S) => r("color-change", S), L = (S) => r("line-width-change", S), Y = (S) => r("text-size-change", S), z = (S) => r("mosaic-size-change", S), N = () => r("undo"), E = () => r("delete"), O = () => r("save"), U = () => r("confirm"), V = () => r("cancel"), ot = (S) => {
        ...
        A("div", vn, [
          ...
          A("div", kn, [
            (j(!0), K(
              Tt,
              null,
              Rt(l.value, (M) => (j(), K("button", {
                key: M.type,
                onClick: (dt) => W(M.type),
                class: bt(["tool-btn", { active: S.currentTool === M.type }]),
                title: M.title
              }, [
                (j(), ie(me(M.icon), {
                  theme: "outline",
              ...
              A("div", Wn, [
                (j(), K(
                  ...
                  Rt(u, (M) => A("button", {
                    key: M,
                    onClick: (dt) => L(M),
                    class: bt(["width-btn", { active: S.currentLineWidth === M }]),
                    title: `${M}px`
                  }, [
                    A(
                      "div",
                      {
                        class: "line-preview",
                        style: xt({ height: `${M}px`, backgroundColor: S.currentColor })
                      },
                      null,
                      4
                      /* STYLE */
                ...
                A("div", Bn, [
                  ...
                  (j(), K(
                    ...
                    Rt(g, (M) => A("button", {
                      key: M,
                      onClick: (dt) => C(M),
                      class: bt(["color-btn", { active: S.currentColor === M }]),
                      style: xt({ backgroundColor: M }),
                      title: M
                    }, null, 14, En)),
                    64
                    /* STABLE_FRAGMENT */
              ...
              A("div", Fn, [
                (j(), K(
                  ...
                  Rt(w, (M) => A("button", {
                    key: M,
                    onClick: (dt) => Y(M),
                    class: bt(["size-btn", { active: S.currentTextSize === M }]),
                    title: `${M}px`
                  }, [
                    A(
                      "span",
              ...
              A("div", Xn, [
                (j(), K(
                  ...
                  Rt(p, (M) => A("button", {
                    key: M,
                    onClick: (dt) => z(M),
                    class: bt(["size-btn", { active: S.currentMosaicSize === M }]),
                    title: `${ft(s)("screenshot.brush")} ${M * 3}px`
                  }, [
                    A(
                      "div",
                      {
                        ...
                        style: xt({
                          width: `${Math.min(M * 1.5, 24)}px`,
                          height: `${Math.min(M * 1.5, 24)}px`,
            ...
            b.value ? (j(), K("div", Zn, [
              A("div", Gn, [
                (j(!0), K(
                  Tt,
                  null,
                  Rt(c.value, (M) => (j(), K("button", {
                    key: M.value,
                    onClick: (dt) => ot(M.value),
                    class: bt(["engine-btn", { active: S.currentTranslateEngine === M.value }]),
                    title: M.label
                  }, Wt(M.short), 11, qn))),
                  128
                  /* KEYED_FRAGMENT */
  ...
  setup(o) {
    ...
    const x = ht({
      ...
      isDrawing: !1
      // 将绘制状态也放到响应式状态中
    }), v = ht(), R = ht(), $ = ct(() => x.value.isDrawing), _ = ct(() => {
    ...
    return ye(async () => {
      if (t.value) {
        ...
        document.addEventListener("keydown", _t), window.addEventListener("resize", I), window.addEventListener("screenshot-toolbar-resize", lt), R.value = await Vt("background-ready", () => {
      ...
      s?.destroy(), s = null, document.removeEventListener("keydown", _t), window.removeEventListener("resize", I), window.removeEventListener("screenshot-toolbar-resize", lt), v.value?.(), R.value?.(), v.value = void 0, R.value = void 0, i.value = null;
    }), (h, k) => (j(), K(
      "div",
      Qn,
      [
        ...
        x.value.selectionRect && !$.value ? (j(), ie(Jn, {
          ...
          onConfirm: M,
          onCancel: dt
        }, null, 8, ["style", "current-tool", "current-color", "current-line-width", "current-text-size", "current-mosaic-size", "can-undo", "can-delete", "current-translate-engine"])) : H("v-if", !0),
        H(" 加载提示 - 仅在初始化时显示 "),
        u.value ? (j(), K("div", ii, [
          k[1] || (k[1] = A(
            "div",
        ...
        !x.value.selectionRect && !u.value ? (j(), K("div", si, [
          A(
            "p",
            null,
            Wt(h.$t("screenshotTool.dragToSelect")),
            1
            /* TEXT */
          ),
          A(
            "p",
            oi,
            Wt(h.$t("screenshotTool.pressEscToClose")),
            1
            /* TEXT */
          )
        ])) : H("v-if", !0),
...
}), wi = /* @__PURE__ */ oe(ai, [["__scopeId", "data-v-40f1d1c6"]]);
export {
  wi as default
};
