...
The first implementation moves are also in place: `translation`, `search-engines`, `desktop-files`, `todo`, `system-theme`, local launcher app/bookmark search, screenshot command adapters, and shared URL opening now live under the Rust plugin modules instead of the older broad implementation modules.
