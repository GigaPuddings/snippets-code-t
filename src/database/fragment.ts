...
// 获取片段列表
export function getFragmentList(id?: number, searchVal: string = ''): Promise<ContentType[]> {
  return new Promise(resolve => {
    console.log('获取片段列表');

    const db = getDb();
    console.log(id, searchVal);
...
export function addFragment(categoryId: number): Promise<number> {
  return new Promise(resolve => {
    ...
    const db = getDb();
    db.then((db) => {
      db.execute(`insert into contents (title,content,category_id,created_at) values('未命名片段','',?,datetime('now', 'localtime'));`, [categoryId]).then((res) => {
...
export function editFragment(params: any): Promise<void> {
  return new Promise(resolve => {
    ...
    const db = getDb();
    db.then((db) => {
      db.execute(`update contents set title= ?,content= ?,category_id= ? where id= ?`, [params.title, params.content, params.category_id, params.id]).then(() => {
        resolve()
      })
    })
  }
