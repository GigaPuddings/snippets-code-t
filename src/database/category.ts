...
// 获取所有分类
export function getCategories(fileSort: 'asc' | 'desc'): Promise<CategoryType[]> {
...
export function editCategory(id: number | string, name: string): Promise<void> {
  return new Promise(resolve => {
    const db = getDb();
    db.then((db) => {
      db.execute(`update categories set name = ? where id = ?`, [name, id]).then(() => {
        resolve()
      })
    })
  })
...
export function addCategory(): Promise<number> {
  return new Promise(resolve => {
    const db = getDb();
    db.then((db) => {
      db.execute(`insert into categories (name, created_at) values ('未命名', datetime('now', 'localtime'))`, []).then((res) => {
