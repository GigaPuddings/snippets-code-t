...
export async function setupGitSyncRuntimeListeners(
  ...
  try {
    listeners.conflict = await listen<{
      conflict_files: string[];
      untracked_files?: string[];
    }>('git-conflict-detected', async (event) => {
      ...
      const conflictFiles = event.payload.conflict_files.map(
        decodeGitConflictFilePath
      );
      const untrackedFiles = (event.payload.untracked_files || []).map(
        decodeGitConflictFilePath
      );
  ...
  try {
    listeners.repoNotFound = await listen<{
      remote_url: string;
      operation: string;
    }>('git-repo-not-found', async (event) => {
...
export function cleanupGitSyncRuntimeListeners(
  listeners: GitSyncRuntimeListeners
): void {
