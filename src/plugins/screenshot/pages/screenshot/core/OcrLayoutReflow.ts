...
const LIST_ITEM_START =
  /^\s*(?:(?:[-*+•·●○■□▪▫◆◇]|[▶▷►▸▹→]\s*(?:[·•]\s*)?)\s+|\d+[.)、）]\s*|[a-zA-Z][.)]\s+)/;
const INLINE_LIST_ITEM_START =
  /(?:^|\s)(?:(?:[-*+•·●○■□▪▫◆◇]|[▶▷►▸▹→]\s*[·•])\s+|\d+[.)、）]\s*|[a-zA-Z][.)]\s+)/g;

export function reflowOcrBlocks(
  blocks: OcrTextBlock[],
  ...
  return blockGroups.flatMap((group) =>
    reflowBlockGroup(group, averageLineHeight)
  );
}

function reflowBlockGroup(
  blocks: OcrTextBlock[],
  ...
  for (const line of lines) {
    ...
    previous.isCodeBlock = Boolean(
      previous.isCodeBlock || line.isCodeLike || line.hasTableColumns
    );
    previous.isStructuredBlock = Boolean(
      previous.isStructuredBlock || line.hasTableColumns
    );
  }
...
function getLayoutBlockGroups(
  ...
  const nodes = preorderLayoutNodes(root).filter(
    (node) => node.units.length > 0
  );
  const groups = nodes
  ...
  return groups.length > 0
    ? groups
    : [sortBlocksForSingleColumn(blocks, averageLineHeight)];
}

function isLikelySingleColumnFlow(
  units: LayoutUnit[],
  averageLineHeight: number
): boolean {
  if (units.length < 2) {
  ...
  for (let index = 1; index < sortedUnits.length; index += 1) {
    ...
    const minWidth = Math.max(
      Math.min(previous.right - previous.left, current.right - current.left),
      1
    );
    const hasStrongHorizontalOverlap = overlapWidth / minWidth >= 0.45;
    const hasNearLeftEdge =
      Math.abs(previous.left - current.left) <= averageLineHeight * 1.5;
...
function getLayoutCutsAndRows(units: LayoutUnit[]): {
  cuts: LayoutCut[];
  rows: LayoutUnit[][];
} {
  ...
  while (unitIndex < units.length) {
    ...
    row.sort(
      (a, b) => a.bbox.left - b.bbox.left || a.bbox.right - b.bbox.right
    );
...
function buildLayoutTree(cuts: LayoutCut[], rows: LayoutUnit[][]): LayoutNode {
  ...
  const completeNode = (node: LayoutNode) => {
    ...
    const maxRowBottom = Math.max(
      ...candidates.map((candidate) => candidate.rowBottom),
      root.rowBottom
    );
    const lowestCandidates = candidates.filter(
      (candidate) => candidate.rowBottom === maxRowBottom
    );
    const parent =
      lowestCandidates.length > 0
        ? lowestCandidates.reduce((best, candidate) =>
            candidate.xRight > best.xRight ? candidate : best
          )
        : root;
  ...
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex += 1) {
    ...
    while (unitIndex < row.length && gaps.length >= 2) {
      ...
      let node = activeNodes.find(
        (item) =>
          almostEqual(item.xLeft, xLeft) && almostEqual(item.xRight, xRight)
      );
      if (!node) {
        node = {
          xLeft,
...
function buildVerticalParagraphs(
  blocks: OcrTextBlock[],
  averageLineHeight: number
): ParagraphBlock[] {
  const averageBlockWidth = positiveNumber(
    average(blocks.map((block) => block.width)),
    averageLineHeight * 0.5
  );
  const sameColumnThreshold = Math.max(
    averageBlockWidth * 1.4,
    averageLineHeight * 0.28
  );
  ...
  for (const block of sortedBlocks) {
    ...
    if (
      currentColumn &&
      Math.abs(centerX - columnCenterX) <= sameColumnThreshold
    ) {
  ...
  return columns.map((column) => {
    ...
    const fontSize = average(
      columnBlocks.map((block) =>
        positiveNumber(block.fontSize, averageLineHeight)
      )
    );
    const lineHeight = average(
      columnBlocks.map((block) => getBlockLineHeight(block))
    );
...
function isLikelyVerticalLayout(
  blocks: OcrTextBlock[],
  averageLineHeight: number
): boolean {
  if (blocks.length < 2) {
  ...
  const verticalBlocks = blocks.filter((block) => {
    ...
    return (
      ...
      block.height > Math.max(18, averageLineHeight * 0.35)
    );
  });
...
function removeIconNoiseBlocks(
  blocks: OcrTextBlock[],
  averageLineHeight: number
): OcrTextBlock[] {
  return blocks
    .map((block) => {
      ...
      if (
        !text ||
        isStandaloneExternalLinkIconNoise(block, averageLineHeight) ||
        isLanguageIconNoise(text)
      ) {
        return null;
      }

      const normalizedText = normalizeOcrMarkerText(
        normalizeIconJoinedText(text, block, averageLineHeight)
      );
...
function isStandaloneExternalLinkIconNoise(
  block: OcrTextBlock,
  averageLineHeight: number
): boolean {
  const text = block.text.trim();
...
function normalizeIconJoinedText(
  text: string,
  block: OcrTextBlock,
  averageLineHeight: number
): string {
...
function buildLines(
  ...
  return lineGroups.map((lineGroup) =>
    createLine(lineGroup.blocks, averageLineHeight)
  );
}
...
function createLine(
  ...
  const lineHeight = average(
    sortedBlocks.map((block) => getBlockLineHeight(block))
  );
...
function shouldMergeLineIntoParagraph(
  ...
  if (
    previousIsListItem &&
    !isLikelyListItemContinuation(previousLine, nextLine, averageLineHeight)
  ) {
    return false;
  }
  ...
  const lineGap =
    nextLine.bbox.y - (previousLine.bbox.y + previousLine.bbox.height);
  if (
    lineGap < -averageLineHeight * 0.25 ||
    lineGap >= averageLineHeight * 1.8
  ) {
    return false;
  }
  ...
  if (
    hasVisualHierarchyBreak(previousLine, nextLine, lineGap, averageLineHeight)
  ) {
    return false;
  }

  if (
    hasParagraphRestartAfterShortLine(
      previousLine,
      nextLine,
      lineGap,
      averageLineHeight
  ...
  if (
    hasVerticalMenuItemBreak(previousLine, nextLine, lineGap, averageLineHeight)
  ) {
    return false;
  }
  ...
  const leftTolerance = Math.max(
    averageLineHeight * 1.2,
    previousLine.lineHeight * 0.9
  );
...
function isLikelyListItemContinuation(
  ...
  const minContinuationIndent = Math.max(
    averageLineHeight * 0.8,
    previousLine.lineHeight * 0.65
  );
...
function joinText(left: string, right: string): string {
  ...
  if (
    last === '-' &&
    ASCII_WORD_EDGE.test(beforeHyphen) &&
    ASCII_WORD_EDGE.test(next)
  ) {
...
function splitStructuredParagraphs(
  ...
  return paragraphs.flatMap((paragraph) =>
    splitStructuredParagraph(paragraph, averageLineHeight)
  );
}
...
function splitStructuredParagraph(
  ...
  return segments.map((text, index) => {
    ...
    const bbox =
      sourceLine?.bbox ||
      splitRectVertically(paragraph.bbox, index, segments.length);
    const blocks = sourceLine?.blocks || [
      createSyntheticBlockFromParagraph(paragraph, text, bbox, index)
    ];
    ...
    return {
      ...
      bbox,
      isCodeBlock: Boolean(
        blocks.some((block) => isLikelyCodeLine(block.text))
      ),
      ...
      fontSize: average(
        blocks.map((block) =>
          positiveNumber(
            block.fontSize,
            paragraph.fontSize || averageLineHeight
          )
        )
      ),
...
function createSyntheticBlockFromParagraph(
  ...
  const source =
    paragraph.blocks[Math.min(index, Math.max(paragraph.blocks.length - 1, 0))];
  return {
    ...(source || {
...
function isLikelyTitleLine(
  ...
  const isEnglishTitle =
    ...
    wordCount <= 8 &&
    isMuchShorterThanNext;
  const isShortCjkTitle =
    /[\u3400-\u4dbf\u4e00-\u9fff]/.test(text) &&
    text.length <= 12 &&
    isMuchShorterThanNext;
...
function hasVisualHierarchyBreak(
  ...
  const relativeGap =
    lineGap / Math.max(smallerFont, averageLineHeight * 0.6, 1);
...
function hasParagraphRestartAfterShortLine(
  ...
  const isShortTail =
    wordCount <= 3 && line.bbox.width < nextLine.bbox.width * 0.35;
  const hasSimilarIndent =
    Math.abs(line.bbox.x - nextLine.bbox.x) <= averageLineHeight * 0.6;
...
function hasVerticalMenuItemBreak(
  ...
  const leftAligned =
    Math.abs(line.bbox.x - nextLine.bbox.x) <= averageLineHeight * 0.35;
  if (!leftAligned) {
    return false;
  }
...
function isShortMenuText(text: string): boolean {
  ...
  const words = trimmed.split(/\s+/).filter(Boolean);
  return (
    words.length <= 3 && trimmed.length <= 32 && !hasEndingPunctuation(trimmed)
  );
}
...
function isLikelyCodeLine(text: string): boolean {
  ...
  if (
    /^(?:const|let|var|return|import|export|function|class|if|else|for|while|try|catch)\b/.test(
      trimmed
    )
  )
    return true;
  ...
  if (
    /^(?:sudo|npm|yarn|pnpm|deno|bun|git|cd|ls|cat|chmod|mkdir|rm|cp|mv|curl|wget|brew|apt|yum|dnf|pip|cargo)\s+\S+/.test(
      trimmed
    )
  )
    return true;
...
function hasTableLikeColumns(
  ...
  for (let i = 1; i < sortedBlocks.length; i++) {
    ...
    if (
      ...
      (gap > averageLineHeight * 0.9 &&
        isTableKeyLike(previous.text) &&
        startsLikeTableDescription(current.text))
    ) {
      largeGaps += 1;
    }
  }
...
function getAverageLineHeight(blocks: OcrTextBlock[]): number {
  return positiveNumber(
    average(blocks.map((block) => getBlockLineHeight(block))),
    16
  );
}
...
function getBlockLineHeight(block: OcrTextBlock): number {
  return positiveNumber(
    block.lineHeight,
    positiveNumber(block.height, positiveNumber(block.fontSize, 16) * 1.2)
  );
}
