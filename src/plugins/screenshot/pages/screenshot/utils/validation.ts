 ...
 * ```
 */
export function isNonEmptyRect(
  rect: Rect | null | undefined,
  minSize: number = 1
): boolean {
  return isValidRect(rect) && rect.width >= minSize && rect.height >= minSize;
}
 ...
 * ```
 */
export function isValidColorInfo(
  colorInfo: ColorInfo | null | undefined
): colorInfo is ColorInfo {
