...
export class MosaicAnnotation extends BaseAnnotation {
  ...
  private renderRealMosaic(
    ...
    if (!MosaicAnnotation.helperCanvas) {
      ...
      MosaicAnnotation.helperCtx = MosaicAnnotation.helperCanvas.getContext(
        '2d',
    ...
    // 9. 在原位置应用马赛克效果
    for (
      let blockY = -gridOffsetY;
      blockY < srcHeight;
      blockY += physicalBlockSize
    ) {
      for (
        let blockX = -gridOffsetX;
        blockX < srcWidth;
        blockX += physicalBlockSize
      ) {
        // 计算当前块的有效范围
  ...
  hitTest(point: Point, tolerance: number = 8): boolean {
    ...
    if (this.data.points.length === 1) {
      return (
        distance(point, this.data.points[0]) <=
        (this.data.mosaicSize || 8) * 1.5 + tolerance
      );
    }
