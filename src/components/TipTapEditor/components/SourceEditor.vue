...
const updateContentWithSelection = (
  value: string,
  start: number,
  end: number
) => {
...
const indentSelection = (textarea: HTMLTextAreaElement, indentText: string) => {
  ...
  updateContentWithSelection(
    localContent.value.slice(0, lineStart) +
      indentedBlock +
...
const outdentSelection = (
  ...
  const lineStart = localContent.value.lastIndexOf('\n', start - 1) + 1;
  const selectedBlock = localContent.value.slice(lineStart, end);
  const outdentedBlock = selectedBlock.replace(
    new RegExp(`^(?:${indentText}| {1,${props.tabSize}}|\\t)`, 'gm'),
    ''
  );
