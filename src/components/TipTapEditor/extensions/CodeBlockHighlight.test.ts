...
describe('code block language detection', () => {
  it.each([
    ...
    [
      '说明文字\nuse test;\nload data local infile "file.txt" into table users;',
      'sql'
    ]
  ])(
    'detects database command blocks for syntax highlighting',
    (code, language) => {
  ...
  it('keeps an explicitly selected language above auto detection', () => {
    expect(resolveCodeBlockLang('python', 'mysqldump --opt test')).toBe(
      'python'
    );
  });
  ...
  it.each([
    ...
    [
      'react',
      'interface Props { name: string }\nconst App = ({ name }: Props) => <main>{name}</main>;',
      'tsx'
    ],
    ['react-ts', 'const App = (): JSX.Element => <main />;', 'tsx'],
    [
      'tauri',
      '#[tauri::command]\nfn greet(name: &str) -> String { name.into() }',
      'rust'
    ],
    [
      'tauri',
    ...
    'resolves framework fence %s to a loaded grammar',
    (fence, code, language) => {
