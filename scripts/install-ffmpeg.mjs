...
async function main() {
  ...
  if (existsSync(EXE_PATH)) {
    ...
    if (exeSize <= MAX_GITHUB_FILE_BYTES) {
      ...
      console.log(
        '[FFmpeg] 检测到超过 GitHub 单文件限制的静态 FFmpeg，改用 shared 构建重新安装。'
      );
    }
...
async function getReleaseAssetUrl() {
  ...
  if (!response.ok) {
    throw new Error(
      `获取 FFmpeg release 失败: ${response.status} ${response.statusText}`
    );
  }
  ...
  const progress = new TransformStream({
    transform(chunk, controller) {
      ...
      if (total > 0) {
        ...
        console.log(
          `[FFmpeg] 已下载 ${(downloaded / 1024 / 1024).toFixed(1)} MB`
        );
      }
...
async function flattenExtractedDirectory(tempDir, targetDir) {
  ...
  for (const entry of entries) {
    ...
    if (
      !['ffmpeg.exe', 'ffprobe.exe'].includes(lowerEntry) &&
  ...
  return new Promise((resolveRun, rejectRun) => {
    ...
    child.on('exit', (code) => {
      if (code === 0) {
        ...
        rejectRun(
          new Error(
            `${basename(command)} ${args.join(' ')} exited with ${code}`
          )
        );
      }
    });
