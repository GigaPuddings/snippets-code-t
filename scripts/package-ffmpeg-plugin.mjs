...
const MANIFEST_PATH = resolve(
  ROOT,
  'plugin-registry/examples/screen-recorder-ffmpeg-resource/plugin.json'
);
...
async function main() {
  ...
  if (!existsSync(exePath)) {
    throw new Error(
      '未找到 ffmpeg.exe，请将 FFmpeg 放到 src-tauri/resources/ffmpeg/ffmpeg.exe'
    );
  }
  ...
  for (const entry of await readdir(SOURCE_DIR, { withFileTypes: true })) {
    ...
    await copyFile(
      join(SOURCE_DIR, entry.name),
      join(ffmpegTargetDir, entry.name)
    );
  }
...
`,
  ...
  console.log(
    '[FFmpeg] 可在应用插件设置中选择该目录安装，或运行 pnpm ffmpeg:release 同步到插件仓库。'
  );
}
